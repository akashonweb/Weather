...
const DISTRICTS_CSV_PATH = "/static/districts_from_geojson.csv";
...
function styleForFeature(feature) {
  const id = featureId(feature);
  const rec = getMapDataEntry(id);
  const cat = rec ? rec.rainfallCategory : "DRY";
  const sel = selectedAreas.includes(id);
  return {
    fillColor: rainfallColor[cat] || rainfallColor.DRY,
    fillOpacity: 0.85,
    color: sel ? "#2ecc71" : "#444444",
    weight: sel ? 3 : 1,
  };
}

// Note: featureId wrapper used by styleForFeature; it matches one above
function featureId(feature) {
